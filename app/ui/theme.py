...
QComboBox {{
    /* 覆盖共享输入控件的 padding，避免 popup 在某些 Windows 主题下多出空白行 */
    padding-top: 0;
    padding-bottom: 0;
}}
QComboBox QAbstractItemView {{
    /* 关键：必须设 padding/margin:0。
       Qt 6 在 popup 视图加了 padding 后会渲染出一条额外的空白行（顶部高亮但无文字），
       留白完全交给 ::item 的 padding 负责。*/
    background: {CARD};
    border: 1px solid {BORDER};
    border-radius: 8px;
    selection-background-color: {PRIMARY};
